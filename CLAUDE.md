...
11. 대화는 한글로 진행.
12. **파일 수정 시 반드시 현재 폴더에 저장된 파일을 먼저 읽고 그 기준으로 수정할 것.** 사용자가 대화에서 첨부한 파일이나 이전 대화 내용이 아니라, 실제 디스크에 있는 최신 파일이 기준이다. 여러 세션에서 동시에 수정하고 있으므로 항상 작업 직전에 파일을 다시 읽어야 한다.
